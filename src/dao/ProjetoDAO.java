...
public class ProjetoDAO {
	...
	public Projeto findById(int id) {
		...
		sql = "SELECT * FROM " + BancoTabela.PROJETO + " INNER JOIN " + BancoTabela.SITUACAO_PROJETO + " WHERE "
				+ BancoTabela.PROJETO + ".id_" + BancoTabela.PROJETO + " = " + id + " AND " + BancoTabela.PROJETO
				+ ".id_situacao = " + BancoTabela.SITUACAO_PROJETO + ".id_situacao_projeto;";
		...
		try {
			...
			projeto.setSituacao(SituacaoProjeto
					.valueOf(resultado.getString(BancoTabela.SITUACAO_PROJETO + ".descricao").toUpperCase()));
	...
	public static ArrayList<Projeto> pesquisarProjetosPorProfessorESituacao(int idProfessor, SituacaoProjeto situacao) {
		...
		try {
			...
			String sql = "Select * from " + BancoTabela.PROJETO + " where " + BancoTabela.PROJETO
					+ ".id_professor = ? and " + BancoTabela.PROJETO + ".id_situacao = ?";
	...
	// Consulta todos os projetos que estao no banco e retorna os que estao
	// disponiveis para o aluno escolher se candidatar
	@SuppressWarnings("finally")
	public ArrayList<Projeto> pesquisarProjetosDisponiveis() {
		...
		sql = "SELECT * FROM " + BancoTabela.PROJETO + " WHERE " + BancoTabela.PROJETO + ".id_situacao = "
				+ "(SELECT id_situacao_projeto FROM " + BancoTabela.SITUACAO_PROJETO + " WHERE descricao = '" + situacao
				+ "')";
		...
		try {
			...
			while (resultado.next()) {
				...
				projeto.setProfessor(
						ProfessorDAO.getInstance().findById(resultado.getInt(BancoTabela.PROJETO + ".id_professor")));
	...
	public static void addProjeto(Projeto projeto) {
		...
		sql = "INSERT INTO " + BancoTabela.PROJETO
				+ " (titulo,descricao,id_professor,id_situacao) values (?, ?, ?, ? )";
	...
	public static Projeto pesquisarProjetoPorIdProjeto(int idProjeto) {
		...
		try {
			...
			String sql = "Select * from " + BancoTabela.PROJETO + " where " + BancoTabela.PROJETO + ".id_projeto = ? ";
	...
	public Projeto findByTitulo(String titulo) {
		...
		sql = "SELECT * FROM " + BancoTabela.PROJETO + " INNER JOIN " + BancoTabela.SITUACAO_PROJETO + " WHERE "
				+ BancoTabela.PROJETO + ".titulo = '" + titulo + "' AND " + BancoTabela.PROJETO + ".id_situacao = "
				+ BancoTabela.SITUACAO_PROJETO + ".id_situacao_projeto;";
		...
		try {
			...
			resultado.next();
			projeto.setId(
					resultado.getInt(BancoTabela.PROJETO + ".id_" + BancoTabela.PROJETO.toString().toLowerCase()));
			projeto.setTitulo(resultado.getString(BancoTabela.PROJETO + ".titulo"));
			projeto.setDescricao(resultado.getString(BancoTabela.PROJETO + ".descricao"));
			projeto.getProfessor().setId(resultado.getInt(BancoTabela.PROJETO + ".id_professor"));
			projeto.setProfessor(ProfessorDAO.getInstance().findById(projeto.getProfessor().getId()));
